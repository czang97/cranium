...
print_one_plot <- function(x = ro_brain_yt_1, title = "Sample Plot"){
    setwd("/Users/czang/Documents/cranium/sample_plot/correctionB/youtoo_1")
    wd <- getwd()
...
ro_brain_yt <- ro_brain_ls_yt[[1]]

plot2d(ro_brain_yt , title = "Youtoo Sample #1 - After PCA-step 1") #output image with dimension: 900*300

print_one_plot(x = ro_brain_yt, title = "Youtoo Sample #1 - After PCA-step 1")
```
...
x_intercept <- (-x.coeff + sqrt(x.coeff^2 - 4* quad.coeff * intercept))/ (2*quad.coeff)
  ...
  if(quad.coeff > 0 ){  #if the quadratic open up (concave up), then the slope of the line is positive
    slope <- abs(y_intercept/x_intercept)
  }else {   #if the quadratic open down (concave down), then the slope of the line is negative
    slope <- -abs(y_intercept/x_intercept)
  }
...
cB_brain_yt <- r.data
print_one_plot(x = cB_brain_yt,  title = "Youtoo Sample #1 - After 1 correction for errorB-step 2")


cB2_brain_yt <- correct_errorB.tbl_brain(cB_brain_yt)
print_one_plot(x = cB2_brain_yt,  title = "Youtoo Sample #1 - After 2 corrections for errorB-step 3")

cB3_brain_yt <- correct_errorB.tbl_brain(cB2_brain_yt)
print_one_plot(x = cB3_brain_yt,  title = "Youtoo Sample #1 - After 3 corrections for errorB-step 4")


cB4_brain_yt <- correct_errorB.tbl_brain(cB3_brain_yt)
print_one_plot(x = cB4_brain_yt,  title = "Youtoo Sample #1 - After 4 corrections for errorB-step 5")

#plot2d(cB4_cA_ro_brain_yt , title = "Youtoo Sample #3 - After correction for errorB-step 3") #output image with dimension: 900*300


cB_brain_yt_inf <- correct_errorB.tbl_brain(cB_brain_yt_inf)
```
